...
<div class="max-w-7xl mx-auto px-6 md:px-12 py-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        ...
        <div class="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm transform transition duration-300 hover:shadow-lg hover:-translate-y-1 flex flex-col">
            <img src="{{ $item['img'] }}" alt="{{ $item['name'] }}" class="h-52 w-full object-cover">

            <div class="flex flex-col p-6 flex-grow">
                <h2 class="text-xl font-semibold text-gray-800 mb-1">{{ $item['name'] }}</h2>
                <p class="text-gray-600 text-sm mb-1">Rp {{ number_format($item['price'], 0, ',', '.') }} &bull; {{ $item['tahun'] }}</p>
                <p class="text-gray-400 text-sm mb-6">{{ $item['km'] }}</p>
